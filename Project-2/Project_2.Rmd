...
# WHO Objectives (Riley)

# Objective 1 - Global Map
To create a global map, the leaflet package was used combined with using map data. I used methods to plot the confirmations and deaths data for each region of the world by identifying each unique region. The sums of confirmations and deaths were taken according to the unique regions in the global data files. The dplyr package was used to quickly calculate the sums, and to make it readable on a map plot. 
...
In Objective 1, hot spots for COVID-19 confirmations and deaths throughout the world were noticed throughout the world map. To further narrow down which countries are most affected by COVID-19, a sorted table was created, with the most affected cases at the top. To create this, the data frames containing the sums of confirmations and deaths for each country in the dataset are arranged by amount of confirmations and deaths in descending order. Since the data frames for both are separate, the table needs to reflect this. Thus, the table have five different columns: a rank column in descending order, two columns for confirmations (countries and number of confirmations), and two columns for deaths (countries and number of deaths). To distinguish between the two pairs of columns, an additional title above both is displayed.
```{r}
